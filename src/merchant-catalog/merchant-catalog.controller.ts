...
/** Writes need Bearer JWT from `POST /auth/merchant/login` or super-admin login. */
...
export class MerchantCatalogController {
  ...
  @ApiOperation({ summary: 'List categories for a merchant (stored in database)' })
  ...
  @ApiTags('Merchant categories')
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard, MerchantAccessGuard)
  @ApiOperation({ summary: 'Create category (merchant owner or super admin)' })
  @ApiParam({ name: 'merchantId', type: String })
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard, MerchantAccessGuard)
  @ApiOperation({ summary: 'Update category (merchant owner or super admin)' })
  @ApiParam({ name: 'merchantId', type: String })
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard, MerchantAccessGuard)
  @ApiOperation({ summary: 'Delete category (merchant owner or super admin)' })
  @ApiParam({ name: 'merchantId', type: String })
  @ApiParam({ name: 'categoryId', type: String })
  ...
  @UseGuards(JwtAuthGuard, MerchantAccessGuard)
  @ApiOperation({
    summary: 'Create product with main image and optional gallery (merchant or super admin)',
  })
  @ApiParam({ name: 'merchantId', type: String })
  @ApiParam({ name: 'categoryId', type: String })
  @ApiConsumes('multipart/form-data')
  ...
  @ApiOperation({
    summary:
      'Update product (JSON). Merchant or super admin. Use imageUrl / extraImageUrls for images.',
  })
  @ApiParam({ name: 'merchantId', type: String })
  ...
  @ApiBearerAuth()
  @UseGuards(JwtAuthGuard, MerchantAccessGuard)
  @ApiOperation({ summary: 'Delete product (merchant owner or super admin)' })
  @ApiParam({ name: 'merchantId', type: String })
  @ApiParam({ name: 'productId', type: String })
